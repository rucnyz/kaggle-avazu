...
sys.stdout.flush()
# 得到exptv_app_or_web，为此前天的app_or_web对应的修正点击率
...
vns_small = ['app_or_web', 'device_ip', 'app_site_id', 'device_model', 'C1', 'C14', 'C17', 'C21',
             'device_type', 'device_conn_type']

dftv = t0.loc[np.logical_and(t0.day.values >= 21, t0.day.values < 32), ['click', 'day', 'id'] + vns_small].copy()
...
for vn in vns:
    ...
    print(vn)
print("-----------------------")
n_ks = {'app_or_web': 100, 'app_site_id': 100, 'device_ip': 10, 'C14': 50, 'app_site_model': 50, 'device_model': 500,
        'device_id': 50,
        'C17': 100, 'C21': 100, 'C1': 100, 'device_type': 100, 'device_conn_type': 100, 'banner_pos': 100,
        'app_site_model_aw': 100, 'dev_ip_app_site': 10}
...
t0['rank_day_dev_ip'] = my_grp_idx(np.add(t0.device_ip.values, t0.day.astype('string').values).astype('str'),
                                   t0.id.values.astype('str'))
t0['rank_app_dev_ip'] = my_grp_idx(np.add(t0.device_ip.values, t0.app_id.values).astype('str'),
                                   t0.id.values.astype('str'))
...
t0['diff_cnt_dev_ip_hour_phour_aw2_prev'] = (
                                                    t0.cnt_device_ip_day_hour.values - t0.cnt_device_ip_day_hour_prev.values) * (
                                                    t0.app_or_web * 2 - 1)
t0['diff_cnt_dev_ip_hour_phour_aw2_next'] = (
                                                    t0.cnt_device_ip_day_hour.values - t0.cnt_device_ip_day_hour_next.values) * (
                                                    t0.app_or_web * 2 - 1)
...
feature_list_dict[feature_list_name] = list_param + \
                                       ['exptv_' + vn for vn in ['app_site_id', 'as_domain',
                                                                 'C14', 'C17', 'C21', 'device_model', 'device_ip',
                                                                 'device_id', 'dev_ip_aw',
                                       ...
                                       ['cnt_diff_device_ip_day_pday',
                                        ...
                                        'exp2_device_ip', 'exp2_app_site_id', 'exp2_device_model',
                                        'exp2_app_site_model',
...
t0tv_mx = t0[feature_list_dict[feature_list_name]].to_numpy()
